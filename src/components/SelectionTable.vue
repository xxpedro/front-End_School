        ...
        <div class="row container pt-5">
            <div class="col-4 d-flex justify-content-center rounded">
                <div class="bg-light radius shadow p-3  mb-5  rounded">
                    <select required class="form-select  mb-5" @change="getByUserByEnrollment" v-model="enrollment" aria-label="Default select example">
                        <option  v-for="(StudentData, index) in StudentData.data" :value="StudentData" :key="index">{{StudentData.enrollment}}</option>
                    </select>
                    <div class="card ml-5 radius" style="width: 18rem;">
                    ...
                    <hr>
                    <h4>Horario de este Estudiante</h4>
                    <hr>
                        ...
                        <tr v-for="(data, index) in TeacherData.data" :key="index">
                            ...
                            <th>
                                <div class=" form-switch">
                                    <input class="form-check-input" :disabled="data.numberOfStudents==data.quota ? '' : disabled" @click="SelecionarMateria(data.id, data.name)" type="checkbox" :id="data.name">
                                </div>
                            </th>
                            <td>{{data.name}}</td>
                            <td>{{data.day}}</td>
                            ...
                            <td>{{data.numberOfStudents}}/{{data.quota}}</td>
                        </tr>
                    </tbody>
                </table>
...
//import buttonComponent from '../components/ButtonComponent.vue'
    export default {
        ...
        methods: {
            ...
            UserData(){
                axios.get('https://localhost:44325/api/Student').then(UserInfo=>{
                    this.StudentData = UserInfo;
                    console.log(UserInfo)
                })
            },
            getByUserByEnrollment(){  
                axios.get(`https://localhost:44325/api/Student/${this.enrollment.enrollment}`).then(UserByEnroll=>{
                    this.UserByEnroll= [];
            ...
            SelecionarMateria(data, name)
            {
                const SelectedMatter={
                    studentId:this.enrollment.id,
                    coursesId:data
                }
                    ...
                    if(isChecked)
                    {
                        if(this.enrollment != '')
                        {
                            axios.post('https://localhost:44325/api/Selection', SelectedMatter).then(validateData=>{
                            alert(validateData.data.message);
                            this.getSelectedMatter(this.enrollment.id);
                            this.GetData();
                            });
                        }
            ...
            {
                axios.get(`https://localhost:44325/api/Selection/${id}`).then(data=>{
            ...
            deleteSelectedMatter(id){
                ...
                axios.post('https://localhost:44325/api/Selection/api/Selection/updateQuota',SelectedMatter).then(()=>{
                    this.getSelectedMatter(this.enrollment.id);
                    this.GetData();
                    alert('Eliminado')
                })
